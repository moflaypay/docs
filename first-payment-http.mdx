...
The status can move from `pending` to a final state such as `completed`, `failed`, or `canceled`. See [Transaction statuses](/api-reference/transaction-statuses) for the full status reference.
