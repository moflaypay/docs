...
- A phone number you can use for a sandbox STK push test
...
Polling `GET /v1/payments/{paymentId}` is useful during development. In production, configure [Moflay webhooks](/webhooks/overview) so your backend receives signed payment events such as `payment.completed` and `payment.failed`.
